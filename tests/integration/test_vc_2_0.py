...
class TestVC_2_0:
    def test_did_full_sdk(self):
        ...
        holder_ecdh_key: ECDHKey = ECDHKey.generate_key(
            EcdhCurveType.P256K.value.curve_name
        )
        issuer_ecdh_key: ECDHKey = ECDHKey.generate_key(
            EcdhCurveType.P256K.value.curve_name
        )
        verifier_ecdh_key: ECDHKey = ECDHKey.generate_key(
            EcdhCurveType.P256K.value.curve_name
        )
        ...
        holder_private_key: PrivateKey = algorithm.bytes_to_private_key(
            EncodeType.HEX.value.decode(
                "8523a4145e347f6eb88e094c7eb8b860915c3b7a121cf1016d841fb20c6cb76e"
            )
        )
        issuer_private_key: PrivateKey = algorithm.bytes_to_private_key(
            EncodeType.HEX.value.decode(
                "3ffdd9a59ff8e79fff5be5052357f6528c20bbeab51b919c4af4ad56c9aa4544"
            )
        )
        verifier_private_key: PrivateKey = algorithm.bytes_to_private_key(
            EncodeType.HEX.value.decode(
                "5759800b84abb50eb8c5ecb63e42f887fda706fc51021ba7257e38e1771f52ba"
            )
        )
        ...
        did_init_nonce: str = EncodeType.HEX.value.encode(
            AlgorithmProvider.generate_random_nonce(32)
        )
        ...
        did_init_sign_result: SignResult = did_init_pm.sign_encrypt(
            issuer_did_key_holder
        )
        ...
        decrypted_did_init_pm: ProtocolMessage = ProtocolMessage.from_json(
            did_init_sign_result.result
        )
        ...
        decrypted_did_auth_pm: ProtocolMessage = ProtocolMessage.from_json(
            did_auth_sign_result.result
        )
        ...
        json_ld_vcr: JsonLdVcr = JsonLdVcr(
            context=context, id_=id_, type_=vc_type, request_claim=request_claim
        )
        ...
        decrypted_credential_request: ClaimRequest = (
            decrypted_credential_request_pm.claim_request
        )
        ...
        vc_claim: dict = {
            ...
            "gender": Claim(
                "female", salt="12341c4b0cbff6bee9118da10d6e85a5", display_value="여성"
            ),
            ...
            "citizenship": Claim(
                True, salt="f2341c4b0cbff6bee9118da10d6e85a5", display_value="내국인"
            ),
            "bank": Claim("신한은행"),
        ...
        consent_url_info_param: InfoParam = InfoParam.for_web_view(
            name="위임 이력 페이지", url="https://example.com/"
        )
        consent_image_info_param: InfoParam = InfoParam.for_image_view(
            name="", data_uri="data:image:png:f0zkel....."
        )
        ...
        id_card_layout_list: Dict[str, List[str]] = {
            "idCardGroup": ["name", "birthDate", "phoneNumber"]
        }
        account_layout_list: Dict[str, List[str]] = {
            "accountGroup": ["bank", "accountNumber"]
        }
        display_layout: DisplayLayout = DisplayLayout(
            [id_card_layout_list, account_layout_list]
        )
        ...
        credential_response_sign_result: SignResult = (
            credential_response_pm.sign_encrypt(
        ...
        decrypted_credential_pm: ProtocolMessage = ProtocolMessage.from_json(
            credential_response_sign_result.result
        )
        ...
        presentation_request_sign_result: SignResult = (
            presentation_request_pm.sign_encrypt(did_key_holder=verifier_did_key_holder)
        )
        ...
        decrypted_presentation_request: ClaimRequest = (
            decrypted_presentation_request_pm.claim_request
        )
