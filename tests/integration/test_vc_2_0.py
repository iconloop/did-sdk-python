...
class TestVC_2_0:
    def test_did_full_sdk(self):
        ...
        holder_did_key_holder: DidKeyHolder = DidKeyHolder(did=holder_did,
                                                           key_id=holder_key_id,
                                                           type=AlgorithmType.ES256K,
                                                           private_key=holder_private_key)
        issuer_did_key_holder: DidKeyHolder = DidKeyHolder(did=issuer_did,
                                                           key_id=issuer_key_id,
                                                           type=AlgorithmType.ES256K,
                                                           private_key=issuer_private_key)
        verifier_did_key_holder: DidKeyHolder = DidKeyHolder(did=verifier_did,
        ...
        did_init_pm: ProtocolMessage = ProtocolMessage.for_request(ProtocolType.DID_INIT,
                                                                   claim_request=did_init_request)
        ...
        did_auth_sign_result: SignResult = did_auth_pm.sign_encrypt(did_key_holder=holder_did_key_holder,
                                                                    ecdh_key=holder_ecdh_key)
        ...
        context: list = ["http://54.180.16.76/score/credentials/v1.json",
        ...
        info_param: Dict[str, InfoParam] = {'description': description_info_param,
        ...
        credential_param: JsonLdParam = JsonLdParam.from_(display_layout=display_layout,
                                                          info=info_param,
                                                          context=context,
                                                          type_=[vc_type],
                                                          claim=vc_claim,
        ...
        # GIVEN a Credential
        credential = Credential(algorithm=issuer_did_key_holder.type.name,
        ...
        vpr: JsonLdVpr = JsonLdVpr.from_(context=["http://54.180.16.76/score/credentials/v1.json"],
                                         id_='test',
        ...
        criteria: VpCriteria = VpCriteria(condition_id='uuid-requisite-0000-1111-2222',
                                          vc=decrypted_credential_pm.message,
                                          param=decrypted_credential_pm.ld_param)

        vp: JsonLdVp = JsonLdVp.from_(context='http://54.180.16.76/score/credentials/v1.json',
